...
class ReciprocalRankFusionStrategy:
    ...
    def fuse(
        ...
        for stage, stage_candidates in stage_rankings.items():
            for rank, candidate in enumerate(stage_candidates, start=1):
                ranks_by_key[_candidate_identity(candidate)].setdefault(stage, rank)
    ...
    def _merge_group(
        ...
        source_stages = list(dict.fromkeys(candidate.source_stage or "unknown" for candidate in group))
